...
For a sensor-backed draft, begin with `limacharlie dr prepare --workspace draft --hostname EXACT_NAME --last 24h --oid OID --output json` (or use `--sid`). This resolves one sensor, calculates timestamps, reads a bounded recent sample and returns focused syntax guidance and observed paths. Use `--event-type` to focus the sample. Ambiguous names require an explicit SID. An empty sample means missing evidence, not absent telemetry.

Write candidate.json and positive.json/negative.json in that workspace, then run `limacharlie dr check --workspace draft --oid OID --output json`. It diagnoses paths and rule envelopes, evaluates each fixture separately, labels captured versus modified fixtures, and records hashes in check.json. `status: tested` proves only those fixture outcomes; inspect grounding warnings and user intent. Fix failures and rerun. Each array condition must hold on the same element: include a negative control with those conditions split across elements. Never present a compile-only or failed artifact as completed.

During agent drafting, CLI policy permits stored-evidence reads and checks; tasking, historical scans, cross-org operations and remote mutations are blocked until the next user request. This is a workflow guardrail, not a sandbox for arbitrary code; use least-privilege credentials for an independent authorization boundary. Preparation and checks never save a remote rule. Deploy only after a separate user request authorizes it, using the exact tested candidate and fixtures and `dr deploy --workspace draft` to verify their hashes.
