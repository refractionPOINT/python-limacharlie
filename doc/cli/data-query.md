...
## Search completion for automation

For known executable names, paths, hashes, domains or IPs, start with indexed
`ioc search` / `ioc batch-search`. Use LCQL for historical predicates or aggregation.

`search run --limit N` counts matching event rows, not timeline/facet envelopes.
It stops at a completed page boundary to preserve continuation; returned rows
can exceed N by the remainder of that page. `--max-pages N` bounds searches
with no matches. Limited checkpoints remain incomplete and can be resumed.

`--summary` returns compact JSON with `complete`, `stop_reason`,
`pages_completed`, `matching_rows_returned`, bounded `rows`,
`displayed_rows_complete`, requested window, reported statistics and continuation.
Full rows and aggregations are written to a JSONL artifact. This is the default
for agent-mode JSON output; `--raw` retains API envelopes. A complete query can
have a truncated display. Partial results never establish absence. Scan counts
are not percentages of wall-clock days covered.
